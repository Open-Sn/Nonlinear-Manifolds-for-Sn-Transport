...
This directory contains the one-dimensional discrete-ordinates neutron-transport benchmark for studying linear and nonlinear-manifold reduced-order models (ROMs). It compares projected/intrusive and inferred/semi-intrusive formulations on linear, polynomial, and tensorial trial manifolds.
...
| `README-1D.md` | Describes the 1-D benchmark and its reproducible workflow. |
...
From the repository root, enter the 1-D case directory and generate or reuse the FOM snapshots:
...
The snapshot and generated Figures 1–5 remain in `1D/`. The ROM script displays plots as well as saving them. In a new headless PowerShell session started at the repository root, use:
...
The generated `.npy` and `.pdf` artifacts are written in `1D/` and ignored by Git.
